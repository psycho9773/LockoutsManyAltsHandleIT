...
eventFrame:SetScript("OnEvent", function(self, event, arg1)
    if event == "ADDON_LOADED" and arg1 == addonName then
        ...
        customInputFrame:SetScale(LMAHI_SavedData.zoomLevel)
        ThrottledUpdateDisplay()
        print("LMAHI Debug: Core.lua loaded successfully")
...
mainFrame:SetScript("OnEvent", function(self, event, arg1)
    if event == "ADDON_LOADED" and arg1 == addonName then
        ...
        print("LMAHI Debug: Normalized charOrder for", #charList, "characters")
        LMAHI.CheckResetTimers()
        LMAHI.CheckLockouts()
        ThrottledUpdateDisplay()
...
SlashCmdList["LMAHIDEBUG"] = function(msg)
    if msg == "resetquests" then
        -- Reset all quest lockouts for all characters
        for charName, lockouts in pairs(LMAHI_SavedData.lockouts) do
            for _, lockout in ipairs(LMAHI.lockoutData.quests or {}) do
                local lockoutId = tostring(lockout.id)
                lockouts[lockoutId] = false
                print("LMAHI Debug: Reset quest", lockout.name, "(ID:", lockout.id, ") for", charName)
            end
        end
        ThrottledUpdateDisplay()
        print("LMAHI Debug: All quest lockouts reset for all characters.")
        return
    end
    print("Debug: Saved Instances")
    for i = 1, GetNumSavedInstances() do
        local name, id, reset, difficulty, lockedState = GetSavedInstanceInfo(i)
        print(string.format("Instance: %s | ID: %d | Reset: %d | Locked: %s", name, id, reset, lockedState))
    end
            ...
            for _, lockoutType in ipairs(LMAHI.lockoutTypes) do
                for _, lockout in ipairs(LMAHI.lockoutData[lockoutType] or {}) do
                    if tostring(lockout.id) == lockoutId then
                        lockoutName = lockout.name
                        break
                    end
                end
            end
            print(string.format("  %s (ID: %s): %s", lockoutName, lockoutId, isLocked))
        end
    end
    print("Debug: Reset Timers")
    print("Last Weekly Reset:", LMAHI_SavedData.lastWeeklyReset)
    print("Last Daily Reset:", LMAHI_SavedData.lastDailyReset)
    print("Debug: Quest Lockouts")
    for charName, lockouts in pairs(LMAHI_SavedData.lockouts) do
        print("Character:", charName)
        for _, lockout in ipairs(LMAHI.lockoutData.quests or {}) do
            local lockoutId = tostring(lockout.id)
            local isCompleted = lockouts[lockoutId]
            local apiStatus = C_QuestLog.IsQuestFlaggedCompleted(lockout.id)
            print(string.format("  Quest: %s (ID: %d) | Saved: %s | API: %s", lockout.name, lockout.id, isCompleted, apiStatus))
        end
    end
end
...
-- Placeholder functions to be defined in Utilities.lua
function LMAHI.SaveCharacterData()
    -- Implemented in Utilities.lua
end
